...
export function DataTableToolbar<TData>({ table }: { table: Table<TData> }) {
	...
	const uniqueFromCity = Array.from(
		table.getColumn('FromCity')?.getFacetedUniqueValues()?.entries() || [],
	).map(([key]) => key)
	const uniqueToCity = Array.from(
		table.getColumn('ToCity')?.getFacetedUniqueValues()?.entries() || [],
	).map(([key]) => key)
	const uniqueStartLocation = Array.from(
		table.getColumn('StartLocation')?.getFacetedUniqueValues()?.entries() || [],
	).map(([key]) => key)
	const uniqueEndLocation = Array.from(
		table.getColumn('EndLocation')?.getFacetedUniqueValues()?.entries() || [],
	).map(([key]) => key)
	...
	return (
		<div className="mb-2 flex justify-between">
			<div className="flex space-x-2 ">
				...
				{/* <DataTableFacetedFilter
					column={table.getColumn('FromCity')}
					title="Từ thành phố"
					options={uniqueFromCity}
				/> */}
				<DataTableFacetedFilter
					column={table.getColumn('ToCity')}
					title="Đến thành phố"
					options={uniqueToCity}
				/>
				<DataTableFacetedFilter
